...
export default class Factory {
  ...
  async buildMany(adapter, num, attrsArray = [], buildOptionsArray = [],
    ...
    if (this.options.afterBuild && buildCallbacks) {
      for (i = 0; i < models.length; ++i) {
        let model = models[i];
        models[i] = await this.options.afterBuild(model, attrsArray, buildOptionsArray);
      }
    }
    return models;
  }
  ...
  async createMany(adapter, num, attrsArray = [], buildOptionsArray = []) {
    ...
    const savedModels = [];
    for (let i = 0; i < attrs.length; ++i) {
      savedModels.push(await adapter.save(model, this.Model));
    }

    if (this.options.afterCreate) {
      for (i = 0; i < savedModels.length; ++i) {
        let model = savedModels[i];
        savedModels[i] = await this.options.afterCreate(model, attrsArray, buildOptionsArray);
      }
    }
